...
try:
    ...
    print(f"Warning: Monitoring disabled due to import error: {e}")
    MONITORING_ENABLED = False
    # Create dummy decorator if monitoring fails
    ...
    <body>
        <div class="container">
            ...
            <div class="endpoint">
                ...
                <p>Get API usage metrics and monitoring data</p>
            </div>
        ...
        <script>
            document.getElementById('predictionForm').addEventListener('submit', async function(e) {
                ...
                try {
                    ...
                    if (response.ok) {
                        const churnProb = (result.prediction.churn_probability * 100).toFixed(1);
                        const prediction = result.prediction.churn_prediction === 1 ? 'WILL CHURN' : 'WILL NOT CHURN';
                        const color = result.prediction.churn_prediction === 1 ? '#dc3545' : '#28a745';
...
def predict_single():
    ...
    try:
        ...
        # Log prediction for monitoring (if enabled)
...
def predict_batch():
    ...
    try:
        ...
        # Log batch predictions (if monitoring enabled)
        if MONITORING_ENABLED:
